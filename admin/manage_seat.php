...
if(isset($_GET['id'])){
	$mov = $conn->query("SELECT * FROM sala where id_sala =".$_GET['id']);
	...
	<div class="col-lg-12">
		<form id="manage-seat">
			<div class="form-group">
				...
				<label for="" class="control-label">Nombre del Sucursal</label>
				<select name="theater_id" required="" class="custom-select browser-default" >
					...
					?>
					<option value="<?php echo $row['id_sucursal'] ?>" <?php echo isset($meta['theater_id']) && $meta['theater_id'] == $row['id_sucursal'] ? 'selected' :'' ?>><?php echo $row['name'] ?></option>
				<?php endwhile; ?>
				</select>
			</div>

			<div class="form-group">
				<label for="" class="control-label">Nombre del Sala</label>
				<input type="text" name="seat_group" class="form-control" value="<?php echo isset($meta['seat_group']) ? $meta['seat_group'] : '' ?>">
			</div>
			<div class="form-group">
				<label for="" class="control-label">Cantidad de Asientos</label>
